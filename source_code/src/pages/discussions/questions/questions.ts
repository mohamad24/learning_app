...
export class QuestionsPage {
  ...
  goToPost(post){
    this.navCtr.push('DiscussionPage',{
      post:post
    });
  }
...
loadPosts(page){
    this.showRetry= false;
     return this.apiService.getDiscussions(page);
}
...
delete(discussion){
  this.presentLoading();
  this.apiService.deleteDiscussion(discussion.discussion_id).subscribe(resp=>{
    this.loading.dismiss();
    if(resp['status']){
      this.utils.presentToast('Discussion deleted');
      this.reloadPosts(1);
    }
...
newDiscussion(){
  let modal = this.modalCtrl.create('AddQuestionPage');
