...
export class DiscussionPage {
  ...
  async ngOnInit() {
   ...
   // this.id = this.navParams.get('id');
    ...
    this.id = this.post.discussion_id;
    ...
    this.loadPosts(1).subscribe(response=>{
      this.posts= response['replies']['data'];
...
private loadPosts(page){
    this.showRetry= false;
     return this.apiService.getDiscussion(this.id,page,);
}
...
loadData(event){
      ...
      this.loadPosts(this.currentPage).subscribe(response=>{
        ...
        this.posts= this.posts.concat(response['replies']['data']);
        console.log(this.posts);

        event.complete();

        //determine if this was the last page    
        let totalPages = Math.ceil((response['replies']['total']/response['replies']['per_page']));
...
send(){
  ...
  this.apiService.createDiscussionReply(data).subscribe(resp=>{
